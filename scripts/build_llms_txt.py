...
def main() -> int:
    ...
    lines += [
        ...
        "## Optional",
        "",
        f"- [Готовый ассистент]({RAW}/scripts/ask.py): `python3 scripts/ask.py \"вопрос\"` — поиск нормы, контекст, ответ со ссылкой на статью и пункт.",
        f"- [Инструкция для агента]({RAW}/CLAUDE.md): как искать статью в этом репозитории и как на неё ссылаться.",
